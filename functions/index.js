...
// Cloud Function to toggle a user's disabled status
exports.toggleUserStatus = functions.https.onRequest((req, res) => {
  ...
  return corsHandler(req, res, async () => {
    try {
      ...
      // Get user ID and admin ID from request
      const { uid, disabled, adminUid } = req.body;
      ...
      if (disabled === undefined) {
        return res.status(400).json({ error: "Missing required parameter: disabled" });
      }
      ...
      try {
        // First check if admin is in the hardcoded super admin list
        const superAdminUIDs = ["yuoaYY14sINHaqtNK5EAz4nl8cc2"]; // Same as in other functions
        let isSuperAdmin = superAdminUIDs.includes(adminUid);
        
        // If not found in hardcoded list, check Firestore
        if (!isSuperAdmin && adminUid) {
      ...
      try {
        // Update user disabled status using Firebase Admin SDK
        await admin.auth().updateUser(uid, { disabled: disabled });
        
        console.log(`User ${uid} disabled status updated to: ${disabled}`);
        
        return res.status(200).json({ 
          success: true, 
          message: `User status updated successfully. User is now ${disabled ? 'disabled' : 'enabled'}.` 
        });
      } catch (authError) {
